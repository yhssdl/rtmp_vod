...
$dirTemplate='../'.$cfg_upload_dir;
$dirlen=strlen($dirTemplate);
...
elseif($action=='saveCus')
{
	...
	ShowMsg("操作成功！","admin_files.php?action=custom");
	exit;
}
elseif($action=='save')
{
...
elseif($action=='savenew')
{
	...
	if($filedir!=$defaultfolder){
		ShowMsg("只能把模板添加在{$defaultfolder}文件夹","admin_files.php?path=".$filedir);
		exit;
	}
