<?php
	...
	function runFlv2Mp4(){
		...
		$csqlStr = "SELECT id,tid,title,user,v_pic,file_name FROM sea_subscribe WHERE stat=2 and tid>0  ORDER BY id ASC LIMIT 1";
		$dsql->SetQuery($csqlStr);
		$dsql->Execute('vod_list');
		while($row=$dsql->GetObject('vod_list'))
		{
			...
			if($cfg_movevodfile){
				...
				if($cfg_screenshot && $row->v_pic=="") $jpgfile = $path."/".date("YmdHi")."_".$row->user."_".fixFileName($row->title).".jpg";
			}else{
