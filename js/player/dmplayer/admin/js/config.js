  ...
  layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element','colorpicker', 'slider'], function(){
   ...
   //执行一个 table 实例
    table.render({
      elem: '#dmlist'
      ,height: 600
      ,url: '/js/player/dmplayer/dmku/?ac=list' //数据接口
      ,title: '用户表'
      ,page: true //开启分页
      ...
      ,cols: [[ //表头
        ...
        {field: [4], title: 'ID',value:[4], width:60, align:'center',fixed: 'left', totalRowText: '合计：'}
        ,{field: [0], title: '视频ID', width:100}
		,{field: [9], title: '发布者', width:100}
    ...
    table.render({
      elem: '#dmreport'
      ,height: 600
      ,url: '/js/player/dmplayer/dmku/?ac=reportlist' //数据接口
      ,title: '用户表'
      ,page: true //开启分页
      //,toolbar: 'default' //开启工具栏，此处显示默认图标，可以自定义模板，详见文档
      //,totalRow: true //开启合计行
      ,cols: [[ //表头
  {field: [2], title: 'ID',value:[4], width:60, align:'center',fixed: 'left',fixed: 'left', totalRowText: '合计：'}
        ,{field: [0], title: '视频ID', width:100}
        ,{field: [3], title: '弹幕内容', width: 300}
        ,{field: [1], title: '举报类型', width:100} 
        ,{field: [4], title: '发送IP', width:100} 
        ,{field: [5], title: '发送时间', width: 110}
  ...
  table.on('toolbar(dmlist)', function(obj){
   ...
   form.on('submit(bond_sumbit)', function (data) {
           ...
           $.ajax({
               type : 'POST',
               data:data.field,
               url :'/js/player/dmplayer/dmku/?ac=edit',  
               success : function (data) {
                   layer.msg('保存完成',{time:1000});
               }
          });
       ...
       form.on('submit(reloadlst_submit)', function (data) {
           ...
           table.render({
              elem: '#dmlist'
              ,height: 500
              ,url: '/js/player/dmplayer/dmku/?ac=so&key='+key //数据接口
              ,title: '用户表'
              //,page: true //开启分页
              //,toolbar: 'default' //开启工具栏，此处显示默认图标，可以自定义模板，详见文档
              //,totalRow: true //开启合计行
      ...
      function delreport (id) {$.ajax({url: "/js/player/dmplayer/dmku/?ac=del&type=report&id="+id,success: function(data){}});}
      function delid (id) {$.ajax({url: "/js/player/dmplayer/dmku/?ac=del&type=list&id="+id,success: function(data){}});}
   });
